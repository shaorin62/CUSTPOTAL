	...
	if pcustcode = "" or pcustcode = null then
		...
		if not objrs_1.eof then
			do until objrs_1.eof
				...
				End If
				sql = sql & " inner join wb_contact_trans f on f.cyear='"&cyear&"'  and a.contidx=f.contidx and (f.isHold='N' or f.isHold='Y')"
...
<h2> <u>매체분류별 집행현황 ('<%=cyear%>)</u> </h2>
	...
	%>
			...
			%>
				<tr height='32'>
					<td  class="hd none" ><%=highclassname%></td>
					<td  width="100" ><%=middleclassname%></td>
					<td  width="100" ><%=jan%></td>
					<td  width="100" ><%=feb%></td>
					<td  width="100" ><%=mar%></td>
					<td  width="100" ><%=apr%></td>
