...
'	next
	...
	Select Case UCase(crud)
		...
		Case "U"
			...
			sql = "update wb_contact_mst set custcode=?, title=?, firstdate=?, enddate=?, comment=?, regionmemo=?, mediummemo=?, totalprice=?, uuser=?, udate=? where contidx=?"
			cmd.commandText = sql
			cmd.parameters.append cmd.createparameter("custcode", adchar, adparaminput, 6, teamcode)
			cmd.parameters.append cmd.createparameter("title", advarchar, adparaminput, 200, title)
			cmd.parameters.append cmd.createparameter("firstdate", adDBTimeStamp, adparaminput, 4, firstdate)
...
'			response.end
			...
			clearparameter(cmd)
					...
					sql = "insert wb_contact_md_dtl "
					sql = sql & " select "&nmdidx&", side, '"&nyear&"','"&nmonth&"', standard, quality from wb_contact_md_dtl where seq in (select max(seq) from wb_contact_md_dtl where mdidx="&rs("mdidx")&" group by side, cyear, cmonth)"
					cmd.commandText = sql
					cmd.execute ,, adExecuteNoRecords
					...
					if dateComp = 1 then
						...
						sql = sql & " select '" & nyear & "', '" & nmonth &"' , "&mdidx&", side, monthly, expense, qty, null, '" & request.cookies("userid") & "', '" & date & "' from wb_contact_account where mdidx = " & nmdidx
						cmd.commandText = sql
						cmd.execute ,, adExecuteNoRecords
					else
		...
		Case "D"
			...
			If rs.eof Then
				...
				If rs2(0) = 0 Then
					...
					cmd.commandText = sql
					cmd.Execute ,, adExecuteNoRecords
					response.write "<script> parent.location.replace('/cust/outdoor/list_contact.asp?cmbcustcode="&orgcustcude&"&cmbteamcode="&orgteamcode&"&cyear="&cyear&"&cmonth="&cmonth&"');</script>"
				Else
...
<!--
	window.parent.opener.location.replace("/cust/outdoor/list_contact.asp?cmbcustcode=<%=orgcustcode%>&cmbteamcode=<%=orgteamcode%>&cyear=<%=cyear%>&cmonth=<%=cmonth%>");
	var crud = "<%=lcase(crud)%>";
