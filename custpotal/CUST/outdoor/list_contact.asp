...
<!--
		...
		function getcontactview(contidx, crud) {
			...
			var url = "/cust/outdoor/popup/view_contact.asp?contidx="+contidx+"&custcode="+custcode+"&teamcode="+teamcode+"&cyear="+cyear+"&cmonth="+cmonth+"&crud="+crud;
			var name = "contactpop";
		...
		function getcontactdelete(arg) {
			...
			if (confirm("선택한 계약에 해당 하는 모든 데이터가 삭제됩니다.\n\n계약을 삭제하시겠습니까?")) {
				processFrame.location.href = "/cust/outdoor/process/db_contact.asp?contidx="+arg+"&custcode=<%=pcustcode%>&teamcode=<%=pteamcode%>&cyear=<%=cyear%>&cmonth=<%=cmonth%>&crud=d";
...
//				sendRequest("/cust/outdoor/process/db_delete_contact.asp", params, _getcontactdelete, "GET");
			}
		}
		...
		function getexcel() {
			...
			location.href = "/cust/outdoor/excel/xls_contact.asp?custcode="+custcode+"&teamcode="+teamcode+"&cyear="+cyear+"&cmonth="+cmonth;
		}
            ...
            <td valign="top" height="516" colspan='2'>
				  ...
				  <thead>
					...
					<!-- 	<th class="hd center" width="70">월지급액</th>
						<th class="hd center" width="67">내수액</th>184  ->   75 +92  /  100+92
						<th class="hd center" width="47">내수율</th> -->
						<th class="hd center" width="167">광고주</th>
						<th class="hd right" width="192">운영팀</th>
					</tr>
					</thead>
					<tbody id='tbody'>
					<%
