...
function Calendar_Click(e) {
	cal_Day = e.title;
	if (cal_Day.length > 6) {
		target.value = cal_Day.replace(/\-/g,"");
	}
	oPopup.hide();
}
...
function Show_cal(sYear, sMonth, sDay) {
	...
	Cal_HTML += "<table id=Cal_Table border=0 bgcolor='#f4f4f4' cellpadding=1 cellspacing=2 width=100% onmouseover='parent.doOver(window.event.srcElement)' onmouseout='parent.doOut(window.event.srcElement)' style='font-size : 12;font-family:굴림;'>";
	...
	for (var optYear=(intThisYear-3); optYear<(intThisYear+4); optYear++) {
...
function Show_cal_M(sYear, sMonth) {
	...
	oPopup.show(pop_left, (pop_top + target.offsetHeight), 180, 99, document.body);
}
