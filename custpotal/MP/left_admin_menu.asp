...
<table width="210" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    <td width="210" height="20" class="deps"   background="/images/menu_bg.gif">메뉴관리</td>
		...
		<%
			Dim objrs2
			sql = " select highcustcode custcode, custname from dbo.sc_cust_hdr where isnull(use_flag,0) = 1 and medflag ='A' order by custname  "
		...
		%>
		  <tr>
			<td  width="210" height="22" class='deps_1' background="/images/menu_bg_over.gif"  id="<%=objrs2("custcode")%>_cust" style="width:150px;cursor:hand" onclick="checkDisplay('<%=objrs2("custcode")%>');  left_menu_getdata('sk_admin_menu.asp','<%=objrs2("custcode")%>' ,'HIGHCUSTCODE');  return false;" ><%=objrs2("custname")%></td>
		  </tr>
