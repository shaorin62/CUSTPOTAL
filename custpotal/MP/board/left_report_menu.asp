<table width="210" border="0" cellspacing="0" cellpadding="0" background="/images/menu_bg.gif">
  ...
  <%
	...
	sql = sql & " dbo.WB_GET_REPORT_PLUSIMAGE(midx) PlusImageCnt"
	sql = sql & " from ( "
	sql = sql & " 	select midx, midx midx1, title, lvl, ref "
	sql = sql & " 	from dbo.wb_menu_mst  "
	sql = sql & " 	where custcode is null "
	sql = sql & " 	and lvl = 1 "
	sql = sql & " 	union all "
	sql = sql & " 	select midx, ref midx1, title, lvl, midx ref "
	sql = sql & " 	from dbo.wb_menu_mst  "
	sql = sql & " 	where custcode is null "
	sql = sql & " 	and lvl = 2 "
	sql = sql & " 	union all "
	sql = sql & " 	select a.midx, b.ref midx1, a.title, a.lvl, a.ref "
...
%>
    <tr  id="<%=objrs("midx")%>"  style="display:block;">
		<td  width='210'  height='20' class="deps" background="/images/menu_bg.gif" id="<%=objrs("midx")%>"  onclick="insertLogmst_src('<%=objrs("midx")%>'); get_page(<%=objrs("midx")%>,'<%=objrs("title")%>','<%=objrs("midx")%>'); get_pageSRC('list.asp','<%=objrs("midx")%>','','midx','<%=objrs("title")%>', 1);" style="cursor:hand"><img src="/images/mms_plus.gif" id="A<%=objrs("midx")%>_B<%=objrs("ref")%>_C<%=objrs("midx1")%>" value="0" border="0" onclick="menuclick1(this.id, <%=objrs("midx")%>);" alt="" <%If objrs("PlusImageCnt") <= 1 Then %> style="cursor:hand;display:none;" <%End if%>>&nbsp;<%=objrs("title")%>&nbsp;<%if objrs("NewImageCnt") <= 7 then %><img src="/images/new.gif" width="21" height="10" border="0" alt=""><%end if%></td>
	</tr>
<% ElseIf objrs("lvl") = 2 then %>
	<tr>
		<td  width='210'  height='20' class="deps_1" background="/images/menu_bg.gif" id="A<%=objrs("midx")%>-B<%=objrs("ref")%>-C<%=objrs("midx1")%>-"  onclick="insertLogmst_src('<%=objrs("midx")%>');get_page(<%=objrs("midx")%>,'<%=objrs("title")%>','<%=objrs("midx")%>');  get_pageSRC('list.asp','<%=objrs("midx")%>','','midx','<%=objrs("title")%>', 1) " style="cursor:hand;display:none;"><img src="/images/mms_plus.gif" id="A<%=objrs("midx")%>_B<%=objrs("ref")%>_C<%=objrs("midx1")%>_"  value="0" border="0" onclick="menuclick2(this.id, <%=objrs("midx")%>);"  alt="" <%If objrs("PlusImageCnt") <= 0 Then %> style="cursor:hand;display:none;" <%End if%>>&nbsp;<%=objrs("title")%>&nbsp;<%if objrs("NewImageCnt") <= 7 then %><img src="/images/new.gif" width="21" height="10" border="0" alt=""><%end if%></td>
	</tr>
<% Else %>
	<tr >
		<td  width='210'  height='20' class="deps_2" background="/images/menu_bg.gif" id="A<%=objrs("midx")%>-B<%=objrs("ref")%>-C<%=objrs("midx1")%>-"  onclick="insertLogmst_src('<%=objrs("midx")%>');get_page(<%=objrs("midx")%>,'<%=objrs("title")%>','<%=objrs("midx")%>');  get_pageSRC('list.asp','<%=objrs("midx")%>','','midx','<%=objrs("title")%>', 1) " style="cursor:hand; display:none;">&nbsp;&nbsp;&nbsp;&nbsp;<%=objrs("title")%>&nbsp;<%if objrs("NewImageCnt") <= 7 then %><img src="/images/new.gif" width="21" height="10" border="0" alt=""><%end if%></td>
	</tr>
<%
		end if
		objrs.movenext
	...
	<tr id="<%=objrs("custcode")%>" style="display:none" >
		<td  width='210'  height='0'  background="/images/menu_bg.gif" >
			<%
				...
				sql = " select custcode custcode, "
				sql = sql & " dbo.sc_get_custname_fun(custcode) custname "
				sql = sql & " from wb_menu_mst "
				sql = sql & " where isnull(custcode,'') <> '' "
				sql = sql & " and dbo.sc_get_highcustcode_fun(custcode) =  '" & objrs("custcode") & "' "
				sql = sql & " and isnull(attr01,'') = '1' "
				sql = sql & " group by custcode order by custname "
			...
			%>
				...
				<%
						sql = "Select  midx, title, lvl,  highmenu, CASE ISNULL(dbo.WB_GET_REPORT_NEWIMAGE(midx),'') WHEN '' THEN '999999' ELSE dbo.WB_GET_REPORT_NEWIMAGE(midx) END NewImageCnt from dbo.wb_menu_mst where isnull(attr01,'') = 1 and  custcode = '" & objrs3("custcode") &"' order by ref , lvl"
				...
				<% else %>
						...
						<tr>
							<td   height='19' class="deps_3" background="/images/menu_bg.gif"  id="<%=objrs4("midx")%>"  onclick="insertLogmst_src('<%=objrs4("midx")%>');get_page(<%=objrs4("midx")%>,'<%=objrs4("title")%>','<%=objrs4("midx")%>');  left_menu_getdata('list.asp','<%=objrs4("midx")%>','<%=objrs3("custcode")%>','timcode','<%=objrs4("title")%>')" style="cursor:hand"><%=objrs4("title")%>11&nbsp;<%if objrs4("NewImageCnt") < "2" then %><img src="/images/new.gif" width="21" height="10" border="0" alt=""><%end if%></td>
						</tr>
							<% else %>
						<tr>
							<td   height='19' class="deps_3" background="/images/menu_bg.gif"  id="<%=objrs4("midx")%>"  onclick="insertLogmst_src('<%=objrs4("midx")%>');get_page(<%=objrs4("midx")%>,'<%=objrs4("title")%>','<%=objrs4("midx")%>');  left_menu_getdata('list.asp','<%=objrs4("midx")%>','<%=objrs3("custcode")%>','timcode','<%=objrs4("title")%>')" style="cursor:hand">&nbsp;&nbsp;&nbsp;&nbsp;<%=objrs4("title")%>22&nbsp;<%if objrs4("NewImageCnt") < "2" then %><img src="/images/new.gif" width="21" height="10" border="0" alt=""><%end if%></td>
						</tr>
						<% end if%>
				<%
					end if
...
<!--
	...
	window.onload = function () {
		...
		get_pageSRC('list.asp',cookiemidx,cookiecustcode,'',cookietitlename, 1);
