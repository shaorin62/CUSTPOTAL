...
<!--
		...
		function getprint() {
			...
			frm.target = "_blank";
			frm.method = "post";
			frm.action = "/mp/outdoor/process/print_report.asp";
			frm.submit();
			cleanToggle();
		...
		function getDonwload() {
			...
			frm.method = "post";
			frm.target = "process";
			frm.action = "/mp/outdoor/process/down_html.asp";
			//getToggle();
			//getToggle2();
			frm.submit();
		}
		...
		function goSearch() {
			var frm = document.forms[0];
			frm.method = "post";
			frm.action = "/mp/outdoor/list_report.asp";
			frm.submit();
		}
		...
		function getconvertfile() {
			...
			frm.target = "process";
			frm.method = "post";
			frm.action = "/mp/outdoor/process/create_html.asp";
			frm.submit();
			cleanToggle();
		}
		...
		function getdeletefile() {
			if (confirm("선택한 계약의 파일을 삭제하시겠습니까?")) {
				...
				frm.target = "process";
				frm.method = "post";
				frm.action = "/mp/outdoor/process/delete_html.asp";
				frm.submit();
			cleanToggle();
			}
		}
