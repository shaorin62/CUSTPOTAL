		...
		Case "D"
			...
			If rs.eof Then
				...
				If rs2(0) = 0 Then
					...
					cmd.commandText = sql
					cmd.Execute ,, adExecuteNoRecords
					response.write "<script> parent.location.replace('/mp/outdoor/list_contact.asp?cmbcustcode="&orgcustcude&"&cmbteamcode="&orgteamcode&"&cyear="&cyear&"&cmonth="&cmonth&"');</script>"
				Else
...
<!--
	window.parent.opener.location.replace("/mp/outdoor/list_contact.asp?cmbcustcode=<%=orgcustcode%>&cmbteamcode=<%=orgteamcode%>&cyear=<%=cyear%>&cmonth=<%=cmonth%>");
	var crud = "<%=lcase(crud)%>";
