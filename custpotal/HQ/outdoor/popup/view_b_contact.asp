...
<!--
	...
	function getcontact() {
		...
		sendRequest("/hq/outdoor/inc/getcontactsummary_b.asp", params, _getcontact, "GET");
	}
	...
	function getcontactdetail () {
		...
		sendRequest("/hq/outdoor/inc/getcontactdetail_b.asp", params, _getcontactdetail, "GET");
	}
	...
	function getmap() {
		// 매체 약도 관리
		var mdidx = "<%=mdidx%>";
		var url = "/hq/outdoor/popup/view_map.asp?mdidx="+mdidx ;
		var name = "winMap";
	...
	function getaccount(mdidx, side) {
		//광고 비용 관리
		var url = "/hq/outdoor/popup/view_account.asp?mdidx="+mdidx+"&side="+side+"&flag=B" ;
		var name = "winAccount";
		var left = screen.width / 2 - 550 / 2;
		var top = 10;
		var opt = "width=550, height=550, resizable=no, scrollbars=no, status=yes, left="+left+"&top="+top;
	...
	function gettheme(mdidx, side) {
		...
		var url = "/hq/outdoor/popup/view_theme.asp?mdidx="+mdidx+"&side="+side+"&highcustcode=<%=highcustcode%>&lastdate="+lastdate+"&cyear=<%=pcyear%>&cmonth=<%=pcmonth%>" ;
		var name = "wintheme";
		var left = screen.width / 2 - 550 / 2;
		var top = 10;
		var opt = "width=550, height=550, resizable=no, scrollbars=no, status=yes, left="+left+"&top="+top;
	...
	function getprint() {
		var url = "/hq/outdoor/print/prt_b_contact2.asp?contidx=<%=pcontidx%>&cyear=<%=pcyear%>&cmonth=<%=pcmonth%>";
		var name = "Print";
...
//		alert(filename);
		process.location.href = "/hq/outdoor/print/convert_html.asp?filename="+filename+"&contidx=<%=pcontidx%>&cyear=<%=pcyear%>&cmonth=<%=pcmonth%>&flag=B";
	...
	function setmedium() {
		...
		var url = "/hq/outdoor/popup/view_b_medium3.asp?contidx=<%=pcontidx%>&cyear="+cyear+"&cmonth="+cmonth+"&side="+side+"&mdidx="+mdidx;
		var name = "medium";
	...
	function getmedium(crud) {
		...
		var url = "/hq/outdoor/popup/view_b_medium.asp?contidx=<%=pcontidx%>&cyear="+cyear+"&cmonth="+cmonth+"&side="+side+"&crud="+crud+"&mdidx="+mdidx;
		var name = "medium";
		var left = screen.width / 2 - 540 / 2;
	...
	function getmedium2() {
		var name = "medium";
		var mdidx = document.getElementById("mdidx").value;
		var url = "/hq/outdoor/popup/view_b_medium2.asp?contidx=<%=pcontidx%>&mdidx="+mdidx;
		var left = screen.width / 2 - 540 / 2;
		var top = screen.height / 2 - 596 / 2;
		var opt = "width=540, height=390, resizable=no, scrollbars=no, status=yes,left="+left+"&top="+top;
		window.open(url, name, opt);
	...
	function getvalidation() {
		...
		if (categoryidx == 9)
		{
			var url = "/hq/outdoor/validation_led.asp?contidx="+contidx ;
		}else if (categoryidx == 10)
		{
			var url = "/hq/outdoor/validation_neon.asp?contidx="+contidx ;
		}else if (categoryidx == 11)
		{
			var url = "/hq/outdoor/validation_board.asp?contidx="+contidx ;
		}else if (categoryidx == 135)
		{
			var url = "/hq/outdoor/validation_etc.asp?contidx="+contidx ;
		}
...
<table width="1024" align="center" style="margin-top:10px;">
	<tr>
	  ...
	  <td width='684'  class="context" id="mediummemo" value="<%=mediummemo %>"><%=mediummemo %></td>
	  <td rowspan='3' width='230'  class="context2" id='map'><%If IsNull(rs("map")) Then response.write "<img src='/images/noimage.gif' width='230' height='190'>" Else response.write "<a href='#' onclick='viewMap(); return false;'><img src='/pds/media/"&rs("map")&"' width='230' height='190' onclick=></a>"%></td>
	</tr>
	<tr>
	  <th class="title">지역특성</td>
	  <td  class="context" id="regionmemo"><%=regionmemo%></td>
	</tr>
	<tr>
	  <th class="title">특이사항</td>
	  <td  class="context" id="comment"><%=comment%></td>
	</tr>
</table>
<!-- // 계약 특성 및 약도 -->
