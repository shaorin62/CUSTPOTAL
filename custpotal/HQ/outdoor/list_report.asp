...
<%
	...
	Dim sql : sql = "select distinct a.contidx, a.custcode, a.title, a.startdate, a.enddate, a.flag, e.report from wb_contact_mst a left outer join wb_contact_md b on a.contidx=b.contidx inner join wb_contact_exe c on b.mdidx=c.mdidx  inner join sc_cust_dtl d on a.custcode = d.custcode left outer join wb_report_mst e on e.contidx=a.contidx and e.cyear='" & cyear & "' and e.cmonth='" & cmonth &"' where  a.custcode like '"&pteamcode&"' and d.highcustcode like '"&pcustcode&"' and c.cyear='"&cyear&"' and c.cmonth='"&cmonth&"' order by a.contidx desc"
...
'	response.write sql
	...
	sql = "select sum(isnull(c.expense,0))  expense, a.region, a.locate, sum(isnull(c.qty,0)) qty, a.contidx, filename, a.mdidx, reportname, a.unit, z.custcode, a.medcode, a.empid from wb_contact_mst z inner join wb_contact_md a on a.contidx=z.contidx  inner join wb_contact_exe c on a.mdidx=c.mdidx left outer join wb_report_dtl d on c.mdidx=d.mdidx and c.cyear = d.cyear and c.cmonth = d.cmonth inner join sc_cust_dtl e on z.custcode=e.custcode where c.cyear='"&cyear&"' and c.cmonth='" &cmonth&"' and z.custcode like '"&pteamcode&"' and e.highcustcode like '"&pcustcode&"' group by a.region, a.locate, a.contidx, filename, a.mdidx, reportname, unit, z.custcode, a.medcode, a.empid "
...
<!--
		...
		function getprint() {
			...
			frm.target = "_blank";
			frm.method = "post";
			frm.action = "/hq/outdoor/process/print_report.asp";
			frm.submit();
			cleanToggle();
		...
		function getDonwload() {
			...
			frm.method = "post";
			frm.target = "process";
			frm.action = "/hq/outdoor/process/down_html.asp";
			//getToggle();
			//getToggle2();
			frm.submit();
		}
		...
		function goSearch() {
			var frm = document.forms[0];
			frm.method = "post";
			frm.action = "/hq/outdoor/list_report.asp";
			frm.submit();
		}
		...
		function getconvertfile() {
			...
			frm.target = "process";
			frm.method = "post";
			frm.action = "/hq/outdoor/process/create_html.asp";
			frm.submit();
			cleanToggle();
		}
		...
		function getdeletefile() {
			if (confirm("선택한 계약의 파일을 삭제하시겠습니까?")) {
				...
				frm.target = "process";
				frm.method = "post";
				frm.action = "/hq/outdoor/process/delete_html.asp";
				frm.submit();
			cleanToggle();
			}
		}
    ...
    <tr>
	  ...
	  <table width="1030" border="0" cellspacing="0" cellpadding="0">
          ...
          <tr>
			...
			<td align='right'> <a href="#" onclick="getconvertfile();return false;"> <img src='/images/icon_report.gif' width='14' height='16' alt='선택한 계약의 파일생성'  align='bottom'> 파일생성 </a>  <a href="#" onclick="getdeletefile(); return false;"><img src='/images/m_remove.gif' width='13' height='12' alt='선택한 계약의 파일삭제' align='bottom'>  파일삭제 </a></td>
          </tr>
          <tr>
...
<iframe src="about:blank" name="process" width=1000 height=1000 frameborder=0></iframe>