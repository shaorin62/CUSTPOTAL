...
'	Dim sql : sql = "select c.contidx, c.title, c.firstdate, c.startdate, c.enddate, isnull(sum(m.monthly),0) as monthly,"
'	sql = sql  & "  c.flag, isnull(max(s.a_val),0) a_val, isnull(max(s.b_val),0) b_val, isnull(max(s.c_val),0) c_val, isnull(max(s.d_val),0) d_val, isnull(max(s.e_val),0) e_val, isnull(max(s.a_val),0) + isnull(max(s.b_val),0) + isnull(max(s.c_val),0) + isnull(max(s.d_val),0) + isnull(max(s.e_val),0) tot, max(s.class) totclass "
'	sql = sql  & " from wb_contact_mst c "
'	sql = sql  & " left outer join sc_cust_dtl d on c.custcode = d.custcode "
'	sql = sql  & " left outer join vw_contact_exe_monthly m on m.contidx = c.contidx and m.cyear+m.cmonth<='"&cyear2&cmonth2&"' and m.cyear+m.cmonth>='"&cyear&cmonth&"' left outer join wb_validation_class s on c.contidx = s.contidx and s.isuse = 1"
...
<!--
		...
		function getexcel() {
			...
			location.href = "/hq/outdoor/excel/xls_validation.asp?custcode="+custcode+"&teamcode="+teamcode+"&strstat="+strstat+"&cyear="+cyear+"&cmonth="+cmonth+"&cyear2="+cyear2+"&cmonth2="+cmonth2;
		}
