...
<%
	...
	If FLAG = "HIGHCUSTCODE" Then
		sql = "select m.midx, m.title, c.custname, m.lvl, m.isfile, iscomment, isemail, mp, ref, isnull(m.attr02,'') attr02, isnull(m.mc,'') mc from dbo.wb_menu_mst m inner join dbo.sc_cust_hdr c on m.custcode = c.highcustcode where isnull(m.attr01,'') =''  and  m.custcode = '"&custcode&"' order by  m.ref  , m.lvl"
	ElseIf FLAG ="CUSTCODE" THEN
		sql = "select m.midx, m.title, c.custname, m.lvl, m.isfile, iscomment, isemail, mp, ref, isnull(m.attr02,'') attr02, isnull(m.mc,'') mc from dbo.wb_menu_mst m inner join dbo.sc_cust_dtl c on m.custcode = c.custcode where isnull(m.attr01,'') ='1' and  m.custcode = '"&custcode&"' order by  m.ref  , m.lvl"
	Else
'		sql = " select midx, title, '' custname, lvl, isfile, iscomment, isemail, mp from dbo.wb_menu_mst where custcode is null  "
		sql = " select midx, title, custname, lvl, isfile, iscomment, isemail, mp, ref , attr02, mc"
		sql = sql & " from ( "
		sql = sql & " 	select midx, midx midx1, title, '' custname, lvl, isfile, iscomment, isemail, mp , ref , isnull(attr02,'') attr02, isnull(mc,'') mc"
		sql = sql & " 	from dbo.wb_menu_mst  "
		sql = sql & " 	where custcode is null "
		sql = sql & " 	and lvl = 1 "
		sql = sql & " 	union all "
		sql = sql & " 	select midx, ref midx1, title, '' custname, lvl, isfile, iscomment, isemail, mp , midx ref , isnull(attr02,'') attr02, isnull(mc,'') mc"
		sql = sql & " 	from dbo.wb_menu_mst  "
		sql = sql & " 	where custcode is null "
		sql = sql & " 	and lvl = 2 "
		sql = sql & " 	union all "
		sql = sql & " 	select a.midx, b.ref midx1, a.title, '' custname, a.lvl, a.isfile, a.iscomment, a.isemail, a.mp , a.ref, isnull(attr02,'') attr02, isnull(mc,'') mc "
		...
		sql = sql & " 	and a.lvl = 3 "
		sql = sql & " ) a "
		sql = sql & " order by midx1, ref, lvl, midx "
	End IF
...
<table width="1024"  border="0" cellpadding="0" cellspacing="0" bordercolor="#8D652B" style="margin-left:3px;">
	<% do until objrs.eof %>
	  <tr class="styleLink" height="31">
		<td width="624" align="left"  class="styleLink" style="padding-left:20px;" onClick="go_menu_view('<%=midx%>')" ><%if lvl = 2 then %><img src="/images/tree-branch.gif" width="19" height="14" border="0" alt="" hspace="5"><% ElseIf lvl = 3 Then %> &nbsp;&nbsp;&nbsp;&nbsp;<img src="/images/tree-branch.gif" width="19" height="14" border="0" alt="" hspace="5"><%end if%><%=title%>&nbsp;</td>
		...
		<td width="3">&nbsp;</td>
			...
			<td width="110" align="center" onClick="go_submenu_reg('<%=midx%>', '<%=attr02%>', '<%=mc%>')" ><%if lvl = 1 Or lvl = 2  then %><img src="/images/btn_submeun_reg.gif" width="100" height="18" border="0" alt="">
			<%End If %>
		<%end if%></td>
	  </tr>
	  <tr>
