...
<table id="Table_01" width="1240" height="80" border="0" cellpadding="0" cellspacing="0">
	<tr>
		<td width="210" height="80" rowspan="2" valign="top"><a href="/hq/main.asp"><img src="/images/main_01.gif" width="210" height="240" alt="go main" border="0"></a></td>
...
'			sql = "select count(ridx) from dbo.wb_report r inner join dbo.wb_menu_mst m on r.midx = m.midx where custcode is null and mp=0"
			...
			Dim totalvisitor
			sql = " select sum(cnt) from wb_account   "
			call get_recordset(objrs, sql)
			if not objrs.eof then
				totalvisitor = objrs(0)
			else
				totalvisitor = 0
			end if
			objrs.close
			...
			sql = " select top "&pagesize&"  ridx, title, cuser, cdate, midx    "
			sql = sql & " from (   "
			sql = sql & " 	select ridx, title, cuser, cdate, midx   "
			sql = sql & " 	from (   "
			sql = sql & " 		select ridx, title, cuser, cdate, midx    "
			sql = sql & " 		from dbo.wb_report   "
			sql = sql & " 		where cdate  >= dateadd(d,-7,getdate())   "
			sql = sql & " 		union all   "
			sql = sql & " 		select r.ridx, r.title, r.cuser, r.cdate, r.midx    "
			sql = sql & " 		from dbo.wb_report r   "
			sql = sql & " 		inner join dbo.wb_menu_mst m on r.midx = m.midx      "
			sql = sql & " 		where r.midx= 1   "
			sql = sql & " 	) a group by ridx, title, cuser, cdate, midx    "
			sql = sql & " ) b    "
  ...
  <tr onMouseOver="this.style.backgroundColor='<%=trgbcolor%>'" onMouseOut="this.style.backgroundColor='#FFFFFF'">
	...
	<span onClick='pop_report_view(<%=ridx%>,<%=midx%>);pop_report_cntupdate(<%=ridx%>,<%=midx%>);' class='styleLink'><% if len(title) > 50 Then response.write Mid(title,1,50) + "..." Else response.write title End If%></span>
	<%
		dim objrs2, objrs3
		...
		if count < 4 then
	%>
	<span onClick="checkForDownload('<%=objrs2("attachfile")%>');pop_report_downcntupdate(<%=ridx%>,<%=midx%>);" class="styleLink" style="padding-left:10px;"> <img src="/images/ico_attach.gif" width="7" height="12" hspace="3"> <%=objrs2("attachfile")%></span>
	<%
		else
		response.write " ..."
		exit do
	...
	<!--   '등록일자를 24시간을 비교해서 작으면 뉴이미지를 찍는다(최신글 리스트)  -->
      <% if datediff("d",c_date,now()) = 1 then %> 
			<img src="/images/i_new.gif" width="12" height="12" align="absmiddle" >
		<% end if %>
    </td>
...
%>
  ...
  <tr>
   ...
   <tr>
    <td colspan="4" height="20" align="right"><font color="#B9B9B9">Total visitor :<font color="#FF6600"><%=totalvisitor%></font></font>&nbsp;&nbsp;</td>
  </tr>
</table>
<!--  -->
</td>
...
<!--
	...
	function pop_report_view(ridx, midx) {
		var url = "/hq/board/pop_report_view.asp?ridx="+ridx+"&midx="+midx+"&read=y";
	...
	function checkForDownload(name) {
		location.href="/hq/board/download.asp?filename="+name;
	}

	function pop_report_cntupdate(ridx, midx) {
		var url = "/hq/board/report_cntupdate_proc.asp?ridx=" + ridx+"&midx="+midx;
		var name = "" ;
		var opt = "width=0, height=0, resizable=no, scrollbars=no, top=0, left=0";
		window.open(url, name, opt);

//			location.href="report_cntupdate_proc.asp?ridx=" + ridx+"&midx="+midx;
	}

	function pop_report_downcntupdate(ridx, midx) {
		var url = "/hq/board/report_downcntupdate_proc.asp?ridx=" + ridx+"&midx="+midx;
		var name = "" ;
